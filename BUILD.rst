Creating SSHLibrary releases
============================

These instructions cover steps needed to create new releases of SSHLibrary.
...
4. Add, commit and push::
      ...
      git add docs/SSHLibrary-$VERSION.rst
      git commit -m "Release notes for $VERSION" docs/SSHLibrary-$VERSION.rst
      git push
...
1. Set version information in `<src/SSHLibrary/version.py>`_::

      invoke set-version $VERSION

2. Commit and push changes::

      git commit -m "Updated version to $VERSION" src/SSHLibrary/version.py
      git push
...
5. Verify that project the page at `PyPI
   <https://pypi.python.org/pypi/robotframework-sshlibrary>`_
   looks good.
...
2. Set dev version based on the previous version::
      ...
      git commit -m "Back to dev version" src/SSHLibrary/version.py
      git push
...
3. SSHLibrary related channels as well as ``#general`` on Slack.
